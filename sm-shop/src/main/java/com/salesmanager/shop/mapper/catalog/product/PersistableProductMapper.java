...
public class PersistableProductMapper implements Mapper<PersistableProduct, Product> {
	...
	public Product merge(PersistableProduct source, Product destination, MerchantStore store, Language language) {
		...
		try {
			...
			if (!CollectionUtils.isEmpty(source.getVariants())) {
				Set<ProductVariant> variants = source.getVariants().stream()
						.map(v -> this.variant(destination, v, store, language)).collect(Collectors.toSet());
			...
			if (source.getInventory() != null) {
				...
				if (!CollectionUtils.isEmpty(destination.getVariants())) {
					...
					for (ProductVariant variant : destination.getVariants()) {
						defaultAvailability = this
								.defaultAvailability(variant.getAvailabilities().stream().collect(Collectors.toList()));
					...
					// create default product availability
					...
					productAvailability.setAvailable(Boolean.valueOf(source.isAvailable()));
					productAvailability.setProduct(destination);

					// create default product price
					ProductPrice productPrice = new ProductPrice();
					productPrice.setDefaultPrice(true);
					productPrice
							.setProductPriceAmount(source.getPrice() != null ? source.getPrice() : new BigDecimal(0));
					productPrice.setCode(ProductPriceEntity.DEFAULT_PRICE_CODE);
					...
					productPriceDescription.setName(ProductPriceEntity.DEFAULT_PRICE_CODE);
					productPriceDescription.setProductPrice(productPrice);
					productPriceDescription.setLanguage(language);
	...
	private ProductVariant variant(Product product, PersistableProductVariant variant, MerchantStore store,
			Language language) {
	...
	private ProductAvailability defaultAvailability(List<ProductAvailability> availabilityList) {
		return availabilityList.stream()
				.filter(a -> a.getRegion() != null && a.getRegion().equals(Constants.ALL_REGIONS)).findFirst().get();
	}
